...
module "create_gke_5_dev3_r3b_subnet_05" {
  ...
  subnetwork         = var.subnet_05_name
  ip_range_pods      = var.subnet_05_secondary_pod_name
  ip_range_services  = var.subnet_05_secondary_svc_1_name
  network_policy     = true
  node_metadata      = "GKE_METADATA_SERVER"
  identity_namespace = "${data.terraform_remote_state.app3_project.outputs.dev3_project_id}.svc.id.goog"
...
# gke-6-apps-r3c-prod - Create GKE zonal cluster in dev3 project using subnet-05 zone b
module "create_gke_6_dev3_r3c_subnet_05" {
  source             = "github.com/terraform-google-modules/terraform-google-kubernetes-engine//modules/beta-public-cluster?ref=v5.1.1"
  project_id         = data.terraform_remote_state.app3_project.outputs.dev3_project_id
  ...
  subnetwork         = var.subnet_05_name
  ip_range_pods      = var.subnet_05_secondary_pod_name
  ip_range_services  = var.subnet_05_secondary_svc_2_name
  network_policy     = true
  node_metadata      = "GKE_METADATA_SERVER"
  identity_namespace = "${data.terraform_remote_state.app3_project.outputs.dev3_project_id}.svc.id.goog"
  monitoring_service = "monitoring.googleapis.com/kubernetes"
...
resource "null_resource" "exec_check_for_cloudbuild_service_accounts_in_app3_project" {
  provisioner "local-exec" {
    command = <<EOT
      for (( c=1; c<=40; c++))
        do
          CHECK=`gcloud projects get-iam-policy ${data.terraform_remote_state.app3_project.outputs.dev3_project_id} --format=json | jq '.bindings[]' | jq -r '. | select(.role == "roles/container.serviceAgent").members[]'`
...
resource "google_project_iam_member" "ops_cloudbuild_sa_gke_admin_in_app3_project" {
  project = data.terraform_remote_state.app3_project.outputs.dev3_project_id
  ...
  depends_on = [
    null_resource.exec_check_for_cloudbuild_service_accounts_in_app3_project
  ]
}

# Give ops Cloudbuild SA clusteradmin role to the ops clusters
resource "null_resource" "exec_gke_clusteradmin_app3" {
  provisioner "local-exec" {
    command = <<EOT
    gcloud container clusters get-credentials "${module.create_gke_5_dev3_r3b_subnet_05.name}" --zone "${var.subnet_05_region}-b" --project "${data.terraform_remote_state.app3_project.outputs.dev3_project_id}"
    gcloud container clusters get-credentials "${module.create_gke_6_dev3_r3c_subnet_05.name}" --zone "${var.subnet_05_region}-c" --project "${data.terraform_remote_state.app3_project.outputs.dev3_project_id}"
    kubectl create clusterrolebinding user-admin-binding --clusterrole=cluster-admin --user=$(gcloud config get-value account) --user=${var.project_editor} --dry-run -oyaml | kubectl apply --context gke_"${data.terraform_remote_state.app3_project.outputs.dev3_project_id}"_"${var.subnet_05_region}-b"_"${module.create_gke_5_dev3_r3b_subnet_05.name}" -f -
    kubectl create clusterrolebinding user-admin-binding --clusterrole=cluster-admin --user=$(gcloud config get-value account) --user=${var.project_editor} --dry-run -oyaml | kubectl apply --context gke_"${data.terraform_remote_state.app3_project.outputs.dev3_project_id}"_"${var.subnet_05_region}-c"_"${module.create_gke_6_dev3_r3c_subnet_05.name}" -f -
    EOT
  ...
  depends_on = [
    module.create_gke_5_dev3_r3b_subnet_05,
    module.create_gke_6_dev3_r3c_subnet_05,
    google_project_iam_member.ops_cloudbuild_sa_gke_admin_in_app3_project,
  ]
}
