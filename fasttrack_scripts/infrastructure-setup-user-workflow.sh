...
mkdir -p $WORKDIR/asm/logs
export LOG_FILE=$WORKDIR/asm/logs/ft-$LAB_NAME-$(date +%s).log
touch $LOG_FILE
exec 2>&1
exec &> >(tee -i $LOG_FILE)
...
print_and_execute "cp ${WORKDIR}/asm/scripts/krompt.bash ${HOME}/.krompt.bash"
...
print_and_execute "$WORKDIR/scripts/setup-gke-vars-kubeconfig.sh"
print_and_execute "source $VARS_FILE"
